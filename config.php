...
define('DB_SERVER', 'localhost');
   define('DB_USERNAME', 'HaiLong');
   define('DB_PASSWORD', '');
   define('DB_DATABASE', 'decor-handmade');
...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Yêu cầu Tài khoản"); }
  if (empty($email)) { array_push($errors, "Yêu cầu Email"); }
  if (empty($password_1)) { array_push($errors, "Yêu cầu Mật khẩu"); }
...
if (isset($_POST['login_user'])) {
  ...
  if (empty($username)) {
  	array_push($errors, "Yêu cầu Tài khoản");
  }
  if (empty($password)) {
  	array_push($errors, "Yêu cầu Mật khẩu");
  }
